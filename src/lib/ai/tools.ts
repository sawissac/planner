...
import { addUser, deleteUser, updateUser } from "@/stores/slices/userSlice";
import type { AppStore } from "@/stores/store";

import { canUndo, undoAction } from "../undo";
...
export const AI_TOOLS: OpenAITool[] = [
      ...
      parameters: {
        ...
        properties: {
          ...
          progress: {
            type: "string",
            description:
              "Must match Available progress states exactly. Use empty string to clear. Setting 'Done' also marks the task done; setting any other value reopens it.",
          },
          groupId: {
            type: "string",
      ...
      parameters: {
        ...
        properties: {
          all: {
            type: "boolean",
            description:
              "True = delete every task in the active file. Use for 'delete the plan' / 'clear everything'.",
          },
          groupId: {
            type: "string",
  ...
  },
  {
    ...
    function: {
      name: "rename_file",
      description: "Rename an existing file. Use the file id from Files in context.",
      parameters: {
        type: "object",
        properties: {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description:
              "Case-insensitive substring match on title and thought. Omit for filter-only search.",
          },
          groupId: { type: "string", description: "Restrict to this group id." },
          assigneeId: {
            type: "string",
            description: "Restrict to tasks assigned to this user id.",
          },
          ...
          dueFrom: {
            type: "string",
            description: "ISO YYYY-MM-DD. Match tasks whose window overlaps on/after this date.",
          },
          dueTo: {
            type: "string",
            description: "ISO YYYY-MM-DD. Match tasks whose window overlaps on/before this date.",
          },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          to: {
            type: "string",
            description: "ISO YYYY-MM-DD. Inclusive end. Default = same as from.",
          },
      ...
      parameters: {
        ...
        properties: {
          ids: {
            type: "array",
            items: { type: "string" },
            description: "Task ids to update. REQUIRED.",
          },
          patch: {
            type: "object",
      ...
      parameters: {
        ...
        properties: {
          ids: {
            type: "array",
            items: { type: "string" },
            description: "Task ids to move. REQUIRED.",
          },
          groupId: {
            type: "string",
            description: "Target group id. Empty string = ungroup. REQUIRED.",
          },
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          title: {
            type: "string",
            description: "Override title. Default: source title + ' (copy)'.",
          },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          name: {
            type: "string",
            description: "New group name. Default: source name + ' (copy)'.",
          },
          includeTodos: {
            type: "boolean",
            description: "Copy tasks from the source group into the new group. Default true.",
          },
        },
        required: ["id"],
      },
      ...
      parameters: {
        ...
        properties: {
          ...
          column: {
            type: "string",
            description:
              "Target column name from Available progress states. Empty string clears progress. REQUIRED.",
          },
        },
        required: ["id", "column"],
      },
      ...
      parameters: {
        ...
        properties: {
          ...
          before: {
            type: "string",
            description:
              "Target neighbor column. The moved column ends up at this column's current index. REQUIRED.",
          },
        },
...
export function runTool(store: AppStore, name: string, argsJson: string): ToolResult {
  ...
  if (name === "add_todo") {
    ...
    const groupId = typeof args.groupId === "string" && args.groupId ? args.groupId : null;
    ...
    const progress = typeof args.progress === "string" && args.progress ? args.progress : null;
  ...
  if (name === "update_todo") {
    ...
    if (typeof args.thought === "string") {
      payload.thought = args.thought.trim();
    }
    ...
    if (typeof args.dueFrom === "string") {
      ...
      if (Number.isFinite(ts)) {
        payload.completedFrom = startOfDay(ts);
      }
    }
    if (typeof args.dueTo === "string") {
      const ts = Date.parse(args.dueTo);
      if (Number.isFinite(ts)) {
        payload.completedTo = endOfDay(ts);
      }
    }
  ...
  if (name === "delete_todos") {
    ...
    if (typeof args.groupId === "string" && args.groupId) {
      ids = file.todos.filter((t) => t.groupId === args.groupId).map((t) => t.id);
    } else if (Array.isArray(args.ids)) {
      ids = (args.ids as unknown[]).filter((x): x is string => typeof x === "string");
    } else {
  ...
  if (name === "delete_group") {
    ...
    if (cascade) {
      ...
      const file = state.todos.files.find((f) => f.id === state.todos.activeFileId);
      if (file) {
        cascadedIds = file.todos.filter((t) => t.groupId === id).map((t) => t.id);
        for (const tid of cascadedIds) {
          store.dispatch(deleteTodo(tid));
        }
      }
    }
  ...
  if (name === "search_tasks") {
    ...
    const priority = typeof args.priority === "string" && args.priority ? args.priority : null;
    const progress = typeof args.progress === "string" && args.progress ? args.progress : null;
    const done = typeof args.done === "boolean" ? args.done : null;
    const fromTs = typeof args.dueFrom === "string" ? startOfDay(Date.parse(args.dueFrom)) : null;
    const toTs = typeof args.dueTo === "string" ? endOfDay(Date.parse(args.dueTo)) : null;
    ...
    const matches = file.todos.filter((t) => {
      ...
      if (assigneeId && !t.assignees.includes(assigneeId)) {
        return false;
      }
      ...
      if (fromTs !== null && t.completedTo < fromTs) {
        return false;
      }
      if (toTs !== null && t.completedFrom > toTs) {
        return false;
      }
      return true;
    });
  ...
  if (name === "list_tasks_by_date") {
    ...
    const toTs = typeof args.to === "string" ? endOfDay(Date.parse(args.to)) : endOfDay(fromTs);
  ...
  if (name === "update_todos") {
    ...
    if (typeof patch.dueFrom === "string") {
      ...
      if (Number.isFinite(ts)) {
        base.completedFrom = startOfDay(ts);
      }
    }
    if (typeof patch.dueTo === "string") {
      const ts = Date.parse(patch.dueTo);
      if (Number.isFinite(ts)) {
        base.completedTo = endOfDay(ts);
      }
    }
    if (typeof patch.done === "boolean") {
      base.done = patch.done;
    }
    for (const id of ids) {
      store.dispatch(updateTodo({ ...base, id }));
    }
  ...
  if (name === "move_todos") {
    ...
    for (const id of ids) {
      store.dispatch(updateTodo({ id, groupId }));
    }
  ...
  if (name === "bulk_add_todos") {
    ...
    if (items.length === 0) {
      return { error: "todos array required" };
    }
    ...
    for (const raw of items) {
      ...
      const assigneeIds = Array.isArray(item.assigneeIds)
        ? (item.assigneeIds as unknown[]).filter((x): x is string => typeof x === "string")
        : [];
      const priority = typeof item.priority === "string" ? item.priority : null;
      const progress = typeof item.progress === "string" && item.progress ? item.progress : null;
      const thought = typeof item.thought === "string" ? item.thought.trim() : "";
      store.dispatch(
        updateTodo({
          id,
          priority,
  ...
  if (name === "find_free_slots") {
    ...
    const capacity = typeof args.capacity === "number" && args.capacity > 0 ? args.capacity : 3;
    const limit = typeof args.limit === "number" && args.limit > 0 ? args.limit : 14;
    const DAY = 86400000;
    ...
    for (let ts = fromTs; ts <= toTs; ts += DAY) {
      ...
      for (const t of file.todos) {
        ...
        if (t.completedFrom <= dayEnd && t.completedTo >= dayStart) {
          load++;
        }
      }
      if (load < capacity) {
  ...
  if (name === "reschedule_overdue") {
    ...
    const targetTs = typeof args.toDate === "string" ? Date.parse(args.toDate) : Date.now();
    ...
    const overdue = file.todos.filter((t) => !t.done && t.completedTo < todayStart);
    for (const t of overdue) {
      store.dispatch(updateTodo({ id: t.id, completedFrom: newFrom, completedTo: newTo }));
    }
    return {
  ...
  if (name === "duplicate_todo") {
    ...
    const title =
      typeof args.title === "string" && args.title.trim()
        ...
        : `${src.title} (copy)`;
    const groupId =
      typeof args.groupId === "string" ? (args.groupId === "" ? null : args.groupId) : src.groupId;
    const action = addTodo(title, groupId);
    store.dispatch(action);
    ...
    const fromTs =
      typeof args.dueFrom === "string"
        ...
        : src.completedFrom;
    const toTs =
      typeof args.dueTo === "string" ? parseDate(args.dueTo, src.completedTo) : src.completedTo;
    store.dispatch(
      updateTodo({
        id: newId,
  ...
  if (name === "move_cards") {
    ...
    for (const id of ids) {
      store.dispatch(updateTodo({ id, progress }));
    }
  ...
  if (name === "rename_board_column") {
    ...
    if (!cols.includes(from)) {
      return { error: `column "${from}" not found` };
    }
  ...
  if (name === "delete_board_column") {
    ...
    const reassignTo =
      typeof args.reassignTo === "string" && args.reassignTo.trim() ? args.reassignTo.trim() : null;
  ...
  if (name === "reorder_board_column") {
    ...
    if (!from || !to) {
      return { error: "name and before required" };
    }
    const cols = store.getState().settings.progressOptions;
    if (!cols.includes(from)) {
      return { error: `column "${from}" not found` };
    ...
    if (!cols.includes(to)) {
      return { error: `column "${to}" not found` };
    }
  ...
  if (name === "get_app_features") {
    return {
      features: [
        ...
        {
          name: "Theming",
          summary: "Dark mode, custom accent color, custom fonts and font sizes.",
        },
        {
          name: "Focus mode",
          summary: "Hides chrome to leave only the task table on screen.",
        },
        {
  ...
  if (name === "duplicate_group") {
    ...
    const newName =
      typeof args.name === "string" && args.name.trim() ? args.name.trim() : `${src.name} (copy)`;
