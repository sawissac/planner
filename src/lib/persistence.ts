...
function migrateTodo(v: unknown): Todo | null {
  ...
  return {
    ...
    completedFrom:
      typeof o.completedFrom === "number" ? o.completedFrom : sod.getTime(),
    completedTo:
...
function isSettingsLike(v: unknown): v is Partial<SettingsState> &
  Pick<SettingsState, "tableFont" | "titleFontSize" | "titleFontWeight" | "sidebarWidth" | "columnSizing"> {
  if (!v || typeof v !== "object") return false
  const o = v as Record<string, unknown>
  return (
...
function migrateSettings(v: Partial<SettingsState>): SettingsState {
  return {
    ...
    priorityOptions:
      Array.isArray(v.priorityOptions) &&
...
export async function hydrate(appStore: AppStore): Promise<void> {
  const [todos, settings, users] = await Promise.all([
    loadTodos(),
    loadSettings(),
    loadUsers(),
  ])
  if (todos) appStore.dispatch(replaceState(todos))
  if (settings) appStore.dispatch(replaceSettings(settings))
  if (users) {
    const { replaceUsers } = await import("./userSlice")
...
export function subscribePersist(appStore: AppStore, delayMs = 300): () => void {
  ...
  return appStore.subscribe(() => {
    ...
    if (state.todos !== lastTodos) {
      ...
      if (todosTimer) clearTimeout(todosTimer)
