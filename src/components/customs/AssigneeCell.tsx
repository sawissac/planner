...
import { updateTodo } from "@/stores/slices/todoSlice";
import { addUser } from "@/stores/slices/userSlice";

const AVATAR_COLORS = ["#92A1C6", "#146A7C", "#F0AB3D", "#C271B4", "#C20D90"];

export function AssigneeCell({ id, assignees }: { id: string; assignees: string[] }) {
  ...
  const assignedUsers = assignees.map((aid) => users.find((u) => u.id === aid)).filter(Boolean) as {
    id: string;
    name: string;
    agenda: string;
  }[];
              ...
              <span className="flex items-center gap-1.5 min-w-0">
                <span className="flex items-center">
                  {assignedUsers.slice(0, 5).map((u, i) => (
                    ...
                    >
                      <Avatar size={20} name={u.id} variant="beam" colors={AVATAR_COLORS} />
                    </span>
                  ))}
                </span>
      ...
      <DropdownMenuContent align="start" className="w-48">
        {users.map((u) => (
          <DropdownMenuCheckboxItem
            ...
            className="gap-2"
          >
            <Avatar size={18} name={u.id} variant="beam" colors={AVATAR_COLORS} />
        ...
        <div className="flex items-center gap-1 px-1.5 py-1" onClick={(e) => e.stopPropagation()}>
          ...
          />
          <Button size="icon-sm" variant="ghost" onClick={addNew} aria-label="Add user">
