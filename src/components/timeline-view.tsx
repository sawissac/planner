...
export function TimelineView() {
  ...
  const { rows, dailyData, domain } = useMemo(() => {
    ...
    for (const g of groupMap.values()) {
      if (g.count > 0) allRows.push(g);
    }
    if (ungrouped.count > 0) allRows.push(ungrouped);
    ...
    const daily: DayDatum[] = Array.from({ length: dayCount }, (_, i) => {
      ...
      let doneCount = 0;
      for (const t of todos) {
        if (t.completedFrom <= de && t.completedTo >= ds) ticketCount++;
        if (
          t.doneAt !== null &&
          t.doneAt >= ds &&
          t.doneAt <= de
  ...
  useEffect(() => {
    ...
    const axisG = svg
      .append("g")
      .attr("transform", `translate(${LEFT_LABEL},${TOP_AXIS - 8})`);

    const spanDays = (domain[1].getTime() - domain[0].getTime()) / 86_400_000;
    const tickCount =
      spanDays <= 14 ? Math.ceil(spanDays) : spanDays <= 90 ? 8 : 10;
    const fmt =
      spanDays <= 60
  ...
  return (
    <div className="rounded-xl border bg-card p-6">
      <div className="mb-1 flex items-baseline gap-2">
        ...
        <div className="text-xs text-muted-foreground">{rows.length} {rows.length === 1 ? "group" : "groups"}</div>
      </div>
