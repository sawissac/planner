...
import { ThoughtDialog, preloadThoughtEditor } from "@/components/todo-row-actions";
...
export function BoardView({
  ...
  const file = useAppSelector((s) =>
    s.todos.files.find((f) => f.id === s.todos.activeFileId),
  );
  const allTodos = useMemo(() => file?.todos ?? [], [file]);
  const todos = useMemo(() => {
    if (from === undefined || to === undefined) return allTodos;
    return allTodos.filter(
      (t) => t.completedFrom <= to && t.completedTo >= from,
    );
  }, [allTodos, from, to]);
  ...
  const groups = useMemo(() => {
    ...
    map.set(UNASSIGNED, []);
    for (const p of options) map.set(p, []);
  ...
  const activeTodo =
    activeType === "todo" && activeId
      ? todos.find((t) => t.id === activeId)
      : null;
  const activeColumnLabel =
    activeType === "column" && activeId ? activeId : null;

  const onDragStart = (e: DragStartEvent) => {
    const id = String(e.active.id);
    const type =
      (e.active.data.current?.type as "todo" | "column" | undefined) ?? null;
    setActiveId(id);
    setActiveType(type);
  };
  ...
  const onDragEnd = (e: DragEndEvent) => {
    ...
    if (type === "column") {
      if (overId === UNASSIGNED || id === overId) return;
      if (!options.includes(id) || !options.includes(overId)) return;
    ...
    if (!todo) return;
    const currentCol =
      todo.progress && groups.has(todo.progress) ? todo.progress : UNASSIGNED;
  ...
  const deleteColumn = (name: string) => {
    ...
    setConfirm({
      ...
      description:
        count > 0
          ? `${count} task(s) will move to Unassigned.`
