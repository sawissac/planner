...
import { UserRowActions } from "@/components/customs/UserRowActions";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { setUserColumnSizing } from "@/stores/slices/settingsSlice";
import { addUser, deleteUser, reorderUser, updateUser, type User } from "@/stores/slices/userSlice";
...
export function UserTable() {
  ...
  } = useTouchRowDrag((from, to) => dispatch(reorderUser({ fromId: from, toId: to })));
  ...
  const columns = useMemo<ColumnDef<User>[]>(
    () => [
      ...
      },
      {
        ...
        minSize: 160,
        cell: ({ row, table }) => (
          <AgendaCell user={row.original} meta={table.options.meta as RowMeta} />
        ),
      },
      {
        id: "actions",
