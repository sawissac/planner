...
import { AssigneeCell } from "@/components/customs/AssigneeCell";
...
import { TodoRowActions } from "@/components/customs/TodoRowActions";
...
import { useTouchRowDrag } from "@/hooks/useTouchRowDrag";
...
import type { User } from "@/stores/slices/userSlice";
...
function todosToMarkdownTable(
  ...
  const header = ["Done", "Task", "Priority", "Progress", "Group", "Assignees", "From", "To"];
...
function todosToMarkdownChecklist(rows: Todo[]): string {
  return rows.map((t) => `- [${t.done ? "x" : " "}] ${t.title}`).join("\n");
}

function todosToCsv(rows: Todo[], groups: Group[], userNameById: (id: string) => string): string {
  const header = [
    "done",
    "title",
    "priority",
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-44">
        {options.map((opt) => (
          ...
          >
            <span className="text-muted-foreground [&_svg]:size-3.5">{opt.icon}</span>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger
        render={
          ...
          >
            Group
            <Filter className={cn("size-3", active ? "text-primary" : "opacity-40")} />
          </button>
        }
      />
      <DropdownMenuContent align="start" className="w-48">
        ...
        <DropdownMenuItem onClick={() => onChange("__none__")} className="flex items-center gap-2">
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-44">
        ...
        {options.map((p) => (
          <DropdownMenuItem key={p} onClick={() => onChange(p)} className="flex items-center gap-2">
    ...
    <div className="absolute top-2 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1 rounded-full border border-border bg-popover/95 backdrop-blur px-1.5 py-1 shadow-lg max-w-[calc(100vw-2rem)]">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="center" className="w-56">
          <DropdownMenuItem onClick={onSelectAll} disabled={allSelected}>
            <Check className="size-3.5" />
            Select all visible
          </DropdownMenuItem>
          <DropdownMenuSeparator />
...
export function TodoTable() {
  ...
  } = useTouchRowDrag((from, to) => dispatch(reorder({ fromId: from, toId: to })));
  const [focusedId, setFocusedId] = useState<string | null>(null);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [selected, setSelected] = useState<Set<string>>(() => new Set());
  useEffect(() => {
    if (focusMode && selected.size > 0) {
      setSelected(new Set());
    }
  ...
  const sortedTodos = useMemo(() => {
    ...
    return [...filteredByProgress].sort((a, b) => {
      for (const { id, desc } of sorting) {
        ...
        } else if (id === "completedIn") {
          cmp = a.completedFrom - b.completedFrom;
        }
        if (cmp !== 0) {
          return desc ? -cmp : cmp;
        }
  ...
  const columns = useMemo<ColumnDef<Todo>[]>(
    () => [
      ...
      },
      {
        ...
        size: 200,
        minSize: 100,
        cell: ({ row }) => <PriorityCell id={row.original.id} priority={row.original.priority} />,
      },
      {
        id: "progress",
        header: () => (
        ...
        size: 200,
        minSize: 100,
        cell: ({ row }) => <ProgressCell id={row.original.id} progress={row.original.progress} />,
      },
      {
        id: "group",
        header: () => (
      ...
      },
      {
        ...
        size: 200,
        minSize: 100,
        cell: ({ row }) => <AssigneeCell id={row.original.id} assignees={row.original.assignees} />,
      },
      {
        id: "completedIn",
        header: ({ table }) => {
  ...
  const bulkSetGroup = (groupId: string | null) => {
    for (const id of selected) {
      dispatch(updateTodo({ id, groupId }));
    }
  };
  ...
  const bulkSetProgress = (progress: string | null) => {
    for (const id of selected) {
      dispatch(updateTodo({ id, progress }));
    }
  };
  ...
  for (const u of usersList) {
    ...
    assigneeState[u.id] = n === 0 ? "none" : n === selectedTodos.length ? "all" : "some";
  }
  ...
  const userNameById = (id: string) => usersList.find((u) => u.id === id)?.name ?? "";
  const bulkCopyMarkdown = () =>
    copyText(todosToMarkdownTable(selectedTodos, groups, userNameById));
  const bulkCopyChecklist = () => copyText(todosToMarkdownChecklist(selectedTodos));
  const bulkCopyCsv = () => copyText(todosToCsv(selectedTodos, groups, userNameById));
    ...
    >
      {selected.size > 0 && (
        <BulkActionBar
          ...
          onSelectAll={() => setSelected(new Set(sortedTodos.map((t) => t.id)))}
          onDelete={bulkDelete}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuItem
              onClick={() =>
                copyText(
                  todosToMarkdownChecklist(table.getFilteredRowModel().rows.map((r) => r.original)),
                )
              }
            >
              ...
              onClick={() =>
                copyText(
                  todosToMarkdownTable(
                    ...
                    groups,
                    (id) => usersList.find((u) => u.id === id)?.name ?? "",
                  ),
                )
              }
            >
            ...
            <DropdownMenuItem
              onClick={() =>
                setSelected(new Set(table.getFilteredRowModel().rows.map((r) => r.original.id)))
              }
            >
                    ...
                    {h.column.getCanResize() && (
                      <div
                        ...
                        className={cn(
                          ...
                          h.column.getIsResizing() ? "bg-primary" : "hover:bg-primary/30",
                        )}
                      />
                    )}
                  </th>
                      ...
                      {row.getVisibleCells().map((cell) => (
                        <td
                          ...
                          className={cn(
                            ...
                            !(focusMode && cell.column.id === "title") && "truncate",
                            focusMode &&
                              [
                                "priority",
                                "progress",
      ...
      >
        ...
        <div className="ml-auto flex items-center gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                ...
                >
                  {pagination.pageSize} / page <ChevronDown className="size-3 opacity-60" />
                </button>
              }
            />
          ...
          <span>
            {pageStart + 1}–{Math.min(pageEnd, filteredRows.length)} of {todoCount} tasks
          </span>
          <button
            type="button"
