...
import { setOpen as setAiOpen } from "@/stores/slices/aiSlice";
import { setDarkMode, setSidebarWidth } from "@/stores/slices/settingsSlice";
import {
  clearAll,
  createFile,
  deleteFile,
...
export function Sidebar({
  ...
  const totalDone = files.reduce((acc, f) => acc + f.todos.filter((t) => t.done).length, 0);
      ...
      >
        <TooltipProvider delay={150}>
          <Tooltip>
            ...
            <TooltipContent side="left">{open ? "Close sidebar" : "Open sidebar"}</TooltipContent>
          </Tooltip>
          <Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="left">{darkMode ? "Light mode" : "Dark mode"}</TooltipContent>
          </Tooltip>
          <Tooltip>
  ...
  return (
    <aside
      ...
      className={cn(
        ...
        isMobile ? "fixed inset-y-0 right-0 z-50 shadow-2xl" : "shrink-0 relative",
              ...
              return (
                ...
                >
                  <div
                    ...
                    onDrop={(e) => {
                      e.preventDefault();
                      const fromId = e.dataTransfer.getData("text/plain") || dragFileId;
                    ...
                    {renaming ? (
                      <input
                        ...
                        onBlur={() => {
                          ...
                          if (n) {
                            dispatch(renameFile({ id: f.id, name: n }));
                          }
                          setRenamingId(null);
                        }}
                        onKeyDown={(e) => {
